...
describe('Integration Test', () => {
        ...
        Promise.resolve((_: PropsWithChildren<{ path?: string }>) => {
          ...
          return (
            <div>
              ...
              <span>extRouteRef1: {extLink1()}</span>
              <span>extRouteRef2: {extLink2({ x: 'a' })}</span>
              <span>extRouteRef3: {extLink3?.() ?? '<none>'}</span>
              <span>extRouteRef4: {extLink4?.({ x: 'b' }) ?? '<none>'}</span>
            </div>
          );
        }),
  ...
  it('runs happy paths', async () => {
    ...
    expect(screen.getByText('extRouteRef1: /')).toBeInTheDocument();
    expect(screen.getByText('extRouteRef2: /foo/a')).toBeInTheDocument();
    expect(screen.getByText('extRouteRef3: /sub1')).toBeInTheDocument();
    expect(screen.getByText('extRouteRef4: /foo/b')).toBeInTheDocument();
  });
  ...
  it('runs happy paths without optional routes', async () => {
    ...
    expect(screen.getByText('extRouteRef1: /')).toBeInTheDocument();
    expect(screen.getByText('extRouteRef2: /foo')).toBeInTheDocument();
    expect(screen.getByText('extRouteRef3: <none>')).toBeInTheDocument();
    expect(screen.getByText('extRouteRef4: <none>')).toBeInTheDocument();
  });
